...
function _service_start() {
    ...
    if _is_service_running; then
        ...
        src_user_hook pre_start_postgresql-9.2
        ...
        run_user_hook post_start_postgresql-9.2
    fi
...
function _service_stop() {
    ...
    if [ -n "$pid" ]; then
        src_user_hook pre_stop_postgresql-9.2
        ...
        run_user_hook post_stop_postgresql-9.2
    else
        if `pgrep -x postgres -u $(id -u)  > /dev/null 2>&1`; then
